 ...
 * 说    明: 无
 ***************************************************************/
static double MPU6050ReadAcc(short *accData)
{
    int ret;
...
{   
    E53SC2Data yundata;
    E53SC2ReadData(&yundata);
    /* 计算x, y, z 轴倾角，返回弧度值*/
    data->X_Angle = acos((yundata.Accel[ACCEL_X_AXIS]) / 1800.0);
    data->Y_Angle = acos((yundata.Accel[ACCEL_Y_AXIS]) / 1800.0);
    data->Z_Angle = acos((yundata.Accel[ACCEL_Z_AXIS]) / 1800.0);
 ...
 * 返 回 值: 无
 ***************************************************************/
double E53SC2ReadData(E53SC2Data *ReadData)
{
    int ret;
