...
#define CONFIG_WIFI_SSID          "荣耀X40i"                            //修改为自己的WiFi 热点账号

#define CONFIG_WIFI_PWD           "123456789"//"0987654321"                        //修改为自己的WiFi 热点密码
...
#define CONFIG_APP_DEVICEID       "64b11c23b84c1334befaf5aa_2023714"//"647ef1cdec46a256bca59aaa_202366"       //替换为注册设备后生成的deviceid

#define CONFIG_APP_DEVICEPWD      "85ffc4e1e38efbac9097fada122e72de"//"c6c5dba43a013159a61ea0c66f6eb186"     //替换为注册设备后生成的密钥
...
typedef struct
{
    osMessageQueueId_t           *app_msg;//消息队列（指针）
    int                          connected;
...
//     ret = IoTAdcRead(IOT_WIFI_IOT_ADC_CHANNEL_4, &data, WIFI_IOT_ADC_EQU_MODEL_8, WIFI_IOT_ADC_CUR_BAIS_DEFAULT, 0xff);
...
static void OLED_init_for_project(void) {
    GpioInit();
    //初始化
    IoTI2cInit(0, 100000); /* baudrate: 100000 */
    IoSetFunc(13,IOT_GPIO_FUNC_GPIO_13_I2C0_SDA);
    IoSetFunc(14,IOT_GPIO_FUNC_GPIO_14_I2C0_SCL);
    OLED_init();
}
...
static int task_sensor_entry(void)//传感器函数对应的线程函数
{
    ...
    float current;
    float T,P;
    IoSetPull(9, IOT_GPIO_EDGE_RISE_LEVEL_HIGH);
    IoSetPull(11, IOT_GPIO_EDGE_RISE_LEVEL_HIGH);
