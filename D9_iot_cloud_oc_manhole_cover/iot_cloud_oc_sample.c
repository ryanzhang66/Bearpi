...
static void deal_mpu6050_cmd(cmd_t *cmd, cJSON *obj_root)
{
    ...
    obj_para = cJSON_GetObjectItem(obj_paras, "Mpu6050");
    if (obj_paras == NULL) {
        cJSON_Delete(obj_root);
    }
...
static void deal_cmd_msg(cmd_t *cmd)
{
    ...
    if (strcmp(cJSON_GetStringValue(obj_cmdname), "Agriculture_Control_Mup6050") == 0) {
...
static void IotMainTaskEntry(void)
{
    ...
    ID1=osTimerNew(SensorTaskEntry, osTimerOnce, NULL, NULL);  //(任务函数名，定时器类型，仅限于osTimerOnce(单次定时器)或osTimerPeriodic，定时器回调函数参数，定时器属性，当前不支持该参数，参数可为NULL )
    if (ID1 != NULL && state == 1)
    {
        timerDelay=10U; //（100U == 1S）
