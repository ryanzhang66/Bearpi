...
int E53IA1ReadData(E53IA1Data *ReadData)
{
    ...
    ret = IoTI2cRead(WIFI_IOT_I2C_IDX_1, (BH1750_ADDR << 1) | 0x01, recv_data, sizeof(recv_data)); // 读取光照传感器(BH1750_ADDR)数据
    if (ret != 0) {
        return -1;
    }
    ReadData->Lux = (float)(((recv_data[0] << DATA_WIDTH_8_BIT) + recv_data[1]) / BH1750_COEFFICIENT_LUX);  //光照强度
    /*由于此时p指向A，因而 p->a == A.a,也就是1*/

    uint8_t data[CHECK_DATA_BUTT]; // data array for checksum verification（校验和验证）
    uint16_t dat, tmp;
    uint8_t SHT3X_Data_Buffer[SHT3X_DATA_BUTT]; // byte(字节) 0,1 is temperature byte 4,5 is humidity(湿度)
    ...
    ret = IoTI2cWriteread(WIFI_IOT_I2C_IDX_1, (SHT30_ADDR << 1) | 0x00, &sht30_i2c_data); // Read SHT30_ADDR sensor data
    if (ret != 0) {
        return -1;
    }
    /* check tem */
